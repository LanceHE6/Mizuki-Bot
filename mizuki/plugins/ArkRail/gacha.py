...
async def draw_img_single(oid_list: list, uid: int or str)-> Path:
    ...
    image.paste(stars_img, (160, 500), mask=stars_img)#星级
    image.paste(pro_img, (320, 750))#职业标志

    font = ImageFont.truetype('simhei', 120)
    draw.text((80, 760), f"{''.join(list(profession)[0:2])}", font=font, fill=(0, 0, 0))#职业文字
    font = ImageFont.truetype('simhei', 100)
    draw.text((480, 760), name, font=font, fill=(0, 0, 0))
    if not await is_op_owned(int(uid), int(oid)):#new标识
        draw.text((310, 880), "NEW", font=font, fill='red')
