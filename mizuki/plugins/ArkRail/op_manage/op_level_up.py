...
async def _(event: GroupAndGuildMessageEvent, state: T_State, args: GroupAndGuildMessage = CommandArg()):
    ...
    # 判断干员是否存在
    if oid == -1:
        await op_level_up.finish(GroupAndGuildMessageSegment.at(event) + f"没有找到名为 {name} 的干员")
    # 判断用户是否拥有该干员
    if not await is_op_owned(uid, oid):
        await op_level_up.finish(GroupAndGuildMessageSegment.at(event) + "您未拥有该干员")
    ...
    if op_level == 90:
        await op_level_up.finish(GroupAndGuildMessageSegment.at(event) + "该干员已满级")
    # 插入绘制干员等级图片函数
    ...
    await op_level_up.send(GroupAndGuildMessageSegment.at(event) + reply)


@op_level_up.got("level", prompt="请发送目标等级")
async def got_level(event: GroupAndGuildMessageEvent, state: T_State, level=Arg("level")):
    if isinstance(level, GroupAndGuildMessage):
        level = int(level.extract_plain_text().split(' ')[0])
    uid = (state["uid"])
    if level < 1 or level > 90 or level < state["op_level"]:
        await op_level_up.finish(GroupAndGuildMessageSegment.at(event) + "非法等级")
    cost = await get_cost_op(state["op_level"], level)
    if cost > state["user_lmc_num"]:
        await op_level_up.finish(GroupAndGuildMessageSegment.at(event) + "龙门币余额不足")

    await change_user_op_level(uid, state["oid"], level)
    await change_user_lmc_num(uid, -cost)
    await op_level_up.finish(
        GroupAndGuildMessageSegment.at(event) + f"已将 {state['op_name']} 升到{level}级\n消耗龙门币:{cost}")
